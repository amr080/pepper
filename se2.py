...
class ExtremeScanner:
    """
    Ultra-aggressive scanner with advanced logging:
    ...
    def scan_port(self, ip_str, port):
        """
        Attempt a TCP connect to ip_str:port, logging progress at DEBUG level.
        Checks for potential new subnets in a minimal HTTP banner parse.
        """
    ...
    def _parse_banner_for_subnets(self, banner_text):
        """
        Toy function to detect strings that look like IPs, convert to /24,
    ...
    def scan_subnets(self, subnets):
        """
        Scan multiple subnets in a row, consolidate results.
        """
        all_open = []
        for snet in subnets:
